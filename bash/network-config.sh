...
myhostname=$(hostname)
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #Here we are finding interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') # lan address is found here
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') # lan hostname is found by using  getent hosts with respect to  variable $lan_address
external_ip=$(curl -s icanhazip.com)  #receiving outside IP address from curl
external_name=$(getent hosts $external_ip| awk '{print $2}')  # by looking into hosts file by $external_ip we can get outward ip 
router_address=$(ip r s default| cut -d ' ' -f 3) #discovering a default router address
router_hostname=$(getent hosts $router_address|awk '{print $2}') # getent hoste and $router_address is used to find the router hostname 
